...
class AssetComputeLogUtils{
    ...
    static redactCredentials(obj, silentRemove=false) {
        return AssetComputeLogUtils.redact(obj, FIELDS_TO_REDACT, silentRemove);
    }
        ...
        Object.keys(obj).forEach(key => {
            if (redactionList.includes(key)) {
                ...
                if ((typeof value === 'object') && (value !== null)) {
                    AssetComputeLogUtils.redact(value, redactionList, silentRemove);
                }
            }
        });
        return obj;
    ...
    /**
     * Log an object and redacts credential fields
     *
    ...
    static log(obj, message){
        const redactedObj = AssetComputeLogUtils.redactCredentials(obj);
        if(message) {
