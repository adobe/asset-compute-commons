    ...
    async sendEvent(type, payload) {
        ...
        try {
            ...
            if (this.fsEventPath) {
                ...
                if(!payload){
                    console.log(`Cannot send event for requestId ${params.requestId} because there is no payload`);
                } else { 
            ...
            if(params && params.requestId){
                console.error(`error sending event for requestId ${params.requestId}: ${e.message}, (details: ${JSON.stringify(e) || e})`);
            } else {
