...
class AssetComputeRegistration {
    constructor(params){
        ...
        if (!params.clientId) {
            throw new ArgumentError("no valid `clientId` request params", "registration");
        }
    ...
    async getJournal(findValidJournalFn){
        ...
        if(!this.journal){
            ...
            const consumers = await this.ioEvents.listConsumerRegistrations(integration.consumerId, integration.applicationId);
