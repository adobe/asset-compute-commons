    ...
    */
    ...
    async activationFinished(metrics={}, sendActivationMetrics=true) {
        if (this.NewRelic) {
            ...
            return Promise.resolve(); // resolve to finish when nothing needs to be done
        }
    }

    static async actionMain(mainFunctionToWrap){
        if(typeof mainFunctionToWrap !== 'function') return mainFunctionToWrap;
