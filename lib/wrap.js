...
function metrics(main) {
    ...
    return NewRelic.instrument(async (params={}) => {
        ...
        try {
            ...
            const allPromises = await Promise.all([metrics.activationStarted(), mainPromise]);

            // return response from main action
            console.log('##################');
            console.log("metrics+main promises");
            ...
            console.log('##################');
            console.log('Finishing activation metrics (from wrapper)');
            console.log('##################');
